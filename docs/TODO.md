...
**Fix:** назначение/снятие роли `admin` — только `superadmin`; admin добавляет только `user`.
...
**Статус:** `[x]` — задокументировано (README, DEPLOY §7); ограничение по ролям не делали
