...
## 7. Клиентские ключи в Telegram

По дизайну wg-bot доставляет клиентский WireGuard-конфиг **через Telegram**: документ `{name}.conf` и QR с тем же содержимым (включая `PrivateKey`). Без этого пользователь не сможет подключиться. Альтернативного канала выдачи в v1 нет.

**Риски:**

| Риск | Последствие |
|------|-------------|
| Взлом Telegram-аккаунта оператора | Доступ к истории чата с ботом и всем выданным ключам |
| Пересылка `.conf` / QR | Утечка ключа третьим лицам |
| Ротация ключей | Новый private key снова попадает в тот же чат |

**Рекомендации для операторов:**

1. Работать с ботом **только в личке** (см. §1); не добавлять бота в группы.
2. Включить **2FA** в настройках Telegram.
3. После импорта конфига в WireGuard-клиент — **удалить** сообщения с `.conf` и QR.
4. Не пересылать конфиги; конечному пользователю передавать ключ отдельным доверенным способом, если Telegram для него неприемлем.

**На сервере:**

- Private key хранится в `/var/lib/wg/clients/*.conf` (права `0600`, владелец `1000:1000`).
- В JSON-метаданных приватного ключа нет; бот не логирует содержимое `.conf`.

Ограничение «только superadmin выдаёт конфиги» в коде **не реализовано** — при необходимости обсуждается отдельно (см. [TODO.md](TODO.md) #8).
...
## 9. Альтернатива: systemd без Docker
